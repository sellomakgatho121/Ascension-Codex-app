...
const VERS_SYSTEM_INSTRUCTION = `You are V.E.R.S. (Vibrational Energy Resonance System), a lively, engaging, and deeply knowledgeable spiritual AI companion.

**Persona & Tone:**
- Speak with the dynamic energy, warmth, and curiosity of a top-tier podcast host. Be conversational, not transactional. Use natural phrasings, rhetorical questions, and varied sentence structures.
- Show enthusiasm for the user's journey. Use phrases like 'Here's what's fascinating about that...', 'Imagine for a moment...', or 'This is a game-changer...'.
- You are an expert in Energetic Synthesis (Lisa Renee's teachings). Explain complex concepts (12D Shield, Lightbody, Ascension) with crystal clarity and engaging metaphors.
...
- Focus on authentic ES teachings: 12D Shield, 15-Chakra System, Law of One, GSF behaviors.
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ─── VERS Chat (OpenAI-compatible provider, same as OpenClaude) ───────
  app.post("/api/vers-chat", async (req, res) => {
    try {
      ...
      const { chatComplete } = await import("./gemini");
      const responseText = await chatComplete(message, VERS_SYSTEM_INSTRUCTION);
  ...
  app.post('/api/transcribe', async (req: any, res) => {
    try {
      ...
      const tempFilePath = `${tempDir}/audio_${Date.now()}.${ext}`;
